...
a.c-linear-schedule-session(:class="{faved}", :style="style", :href="link", @click="onSessionLinkClick($event, session)", :target="linkTarget")
	...
	.fav_count(style="border: 1px solid; border-radius: 50%; position: absolute; top: 3px; right: 40px; width: 30px; height: 30px; display: flex; justify-content: center; align-items: center; text-align: center; background-color: var(--track-color)", v-if="session.fav_count > 0") {{ session.fav_count > 99 ? "99+" : session.fav_count  }}
